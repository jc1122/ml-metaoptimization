...
The orchestrator enqueues exactly one immutable batch manifest per experiment batch, doing so mechanically from the `write_manifest` and `enqueue_batch` executor directives emitted by `metaopt-remote-execution-control`.
